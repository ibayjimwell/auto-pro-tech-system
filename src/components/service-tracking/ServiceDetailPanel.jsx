...
import { serviceTrackingStore } from "@/services/serviceTracking";
...
export default function ServiceDetailPanel({
  ...
  const loadSession = useCallback(async () => {
    const s = await serviceTrackingStore.getSession(
      appointment.id,
      appointment,
    );
    setSession(s);
  ...
  const handleAddProduct = async (taskId, product) => {
    ...
    await serviceTrackingStore.updateTask(appointment.id, taskId, {
      addedProducts: updated,
    });
    await refreshSession();
  };
  ...
  const handleUpdateAdditionalItem = async (itemId, data) => {
    await serviceTrackingStore.updateAdditionalItem(
      appointment.id,
      itemId,
      data,
    );
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start gap-3">
        <Button
          ...
          size="sm"
          onClick={onBack}
          className="flex-shrink-0 mt-0.5 h-8 px-2"
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1 flex-wrap">
            ...
            {appointment.appointmentTime && (
              <span className="flex items-center gap-1 text-xs text-muted-foreground">
                <Clock className="w-3 h-3" /> {appointment.appointmentDate} ·{" "}
                {appointment.appointmentTime}
              </span>
            )}
          </div>
          ...
          {TRACKING_STATUSES.map((s, i) => {
            ...
            return (
              <React.Fragment key={s}>
                ...
                >
                  <div
                    className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors flex-shrink-0 ${done ? "bg-primary" : active ? "bg-primary ring-4 ring-primary/20" : "bg-muted"}`}
                  >
                    {done ? (
                      <CheckCircle2 className="w-4 h-4 text-white" />
                    ) : (
                      <span
                        className={`text-xs font-bold ${active ? "text-white" : "text-muted-foreground"}`}
                      >
                        {i + 1}
                      </span>
                    )}
                  ...
                  <span
                    className={`text-[9px] text-center leading-tight hidden sm:block ${active ? "font-bold text-primary" : done ? "text-muted-foreground" : "text-muted-foreground"}`}
                  >
                    {STATUS_LABELS[s]}
                  </span>
                </button>
                {i < TRACKING_STATUSES.length - 1 && (
                  <div
                    className={`h-0.5 flex-1 transition-colors ${done ? "bg-primary" : "bg-muted"}`}
                  />
                )}
              </React.Fragment>
            );
