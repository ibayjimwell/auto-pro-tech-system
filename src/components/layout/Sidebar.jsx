...
import {
  LayoutDashboard, Users, Car, CalendarDays, Wrench, UserCog,
  Activity, FileText, ChevronLeft, ChevronRight, LogOut, Cog, X, Package
} from 'lucide-react';
      ...
      <div className="p-4 flex items-center gap-3 border-b border-sidebar-border min-h-[64px]">
        ...
        {!isCollapsed && (
          <div className="overflow-hidden flex-1">
            <h1 className="font-heading text-lg font-bold text-secondary leading-tight">AUTO PRO TECH</h1>
            <p className="text-[10px] text-sidebar-foreground/50 uppercase tracking-widest">AutoCare System</p>
          </div>
        )}
        {/* Close button on mobile */}
        {onMobileClose && (
          <button onClick={onMobileClose} className="ml-auto p-1 text-sidebar-foreground/50 hover:text-sidebar-foreground lg:hidden">
            <X className="w-5 h-5" />
          </button>
        )}
      </div>

      <NavLinks items={visibleItems} collapsed={isCollapsed} onNavigate={onMobileClose} />
