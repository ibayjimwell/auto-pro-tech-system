  ...
  return (
    <header className="h-16 bg-card border-b border-border px-4 md:px-6 flex items-center justify-between sticky top-0 z-10">
      <div className="min-w-0">
        <h2 className="font-heading text-lg md:text-xl font-bold text-foreground truncate">
          {title}
        </h2>
        {subtitle && (
      ...
      <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-3 pl-2 md:pl-4 border-l border-border">
          ...
          <div className="hidden md:block">
            ...
            <Badge
              variant="outline"
              className="text-[10px] px-1.5 py-0 capitalize border-secondary text-secondary"
            >
              {user?.role || "guest"}
            </Badge>
          </div>
