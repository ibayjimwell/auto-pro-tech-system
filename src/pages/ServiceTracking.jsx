...
const FILTER_OPTIONS = [
  ...
  { value: 'WAITING_FOR_APPROVAL', label: 'Waiting', icon: Timer },
  { value: 'IN_PROGRESS', label: 'In Progress', icon: Wrench },
];

/**
 * ServiceTracking Component
 * Modernized UI for tracking active vehicle service lifecycle.
 */
export default function ServiceTracking() {
  ...
  useAppointmentsSocket(({ type, appointment }) => {
    if (['statusChanged', 'created', 'cancelled', 'updated'].includes(type)) {
  ...
  // --- Conditional Rendering: Details Panel ---
  if (selected) {
    return (
      <ServiceDetailPanel
    ...
    >
      ...
      {filteredAppointments.length === 0 ? (
        <EmptyState
          ...
          description={`There are currently no appointments marked as ${FILTER_OPTIONS.find(f => f.value === activeFilter)?.label.toLowerCase()}.`}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAppointments.map((appt) => (
            ...
            >
              {/* Top Accent Gradient (Changes based on status) */}
                ...
                <div className="p-4 rounded-2xl bg-slate-50/80 border border-slate-100 space-y-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-700 truncate">{appt.serviceType?.name || 'General Service'}</p>
                    </div>
                  </div>
